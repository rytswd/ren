...
try ren.render.fadeIn(allocator, &writer, spaced, .{});  // Animated render
#+end_src
...
- **Fade-in**: ~fadeIn(allocator, writer, block, config)~ - space-reserving fade
- **Typewriter**: ~typewriter(writer, block, config)~ - character-by-character reveal
- **Glow**: ~glow(allocator, writer, block, config)~ - border illumination effect
...
pub fn main() !void {
    ...
    // Or with animation
    // try ren.render.fadeIn(allocator, writer, boxed, .{});
}
#+end_src
...
pub fn main() !void {
    ...
    try ren.render.fadeIn(allocator, writer, outer, .{
        .steps = 10,
