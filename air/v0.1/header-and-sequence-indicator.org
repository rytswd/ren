...
#+state: draft
#+FILETAGS: :box:feature:

* Summary
Implement sophisticated header styles for terminal output: progress indicators for sequential operations and star headers for single operations. These headers provide refined visual hierarchy with full user configurability for colours, widths, and styling.
...
- Star headers for single operations with contextual information
...
- Completed steps: `●` - steps already done
- Current step: `●` - step in progress
- Upcoming steps: `○` - steps not yet started
- Separator line: `─` extends to terminal width
- Step counter: `[ current / total ]`
- Title: Centred below dots (or left-aligned if too long)
- Full separator line below title
- Terminal-width aware (detect width, default 60 if unavailable)

** Star Header for Single Operations
...
Visual design (default styling, all configurable):
- Star: `⚝` - outlined star character
- Separator line: `─` extends to terminal width
- Top right label: `[ ren ]` or contextual info
- Title: Centred below star
- Full separator line below title
...
- Terminal width detection (POSIX ioctl, Windows Console API)
- Colour code utilities (RGB truecolor, 256-colour, 16-colour ANSI)
- Terminal capability detection (truecolor support, colour support)
- Unicode width calculation

* History
- 2025-12-04: Initial draft created, inspired by Air CLI UI modernization
